...
class MalcompletedSpider(scrapy.Spider):
    ...
    def login_n_navigate_n_parse(self, response):
        ...
        # you can navigate to the page just by extracting the link and using SeleniumRequest/driver.get() but here we used this process just to demonstrate the process.In the case of using driver.get(), bear in mind that the selenium response object from driver doesn't work without selenium commands
        try:
            WebDriverWait(driver, 10).until(
